...
# Patch YOLO classes for YOLOv11 compatibility with older ultralytics packages
...
while True:
    ...
    for i in range(len(detections)):
        ...
        if conf > 0.5:
            ...
            # Example: assuming frame is already captured
            height, width, _ = frame.shape

            # Auto scale factor based on resolution
            scale = max(0.5, min(width, height) / 640)  # Base scaling on 640px

            color = bbox_colors[classidx % len(bbox_colors)]

            # Draw the bounding box
            cv2.rectangle(frame, (xmin, ymin), (xmax, ymax), color, int(2 * scale))

            # Create label
            label = f'{classname}: {int(conf * 100)}%'

            # Dynamic font scale and thickness
            ...
            # Make sure label box doesn't go out of the frame
